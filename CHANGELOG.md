...
- **`verve.gl` — skeletal skinning, slice 2 (keyframe animation)** (`src/core/gl/vmesh.zig`
  v6, `gltf.zig`, `math.zig`, `fixture.zig`, `tools/gl_asset_gen.zig`,
  `src/client/islands/GlSkin.zig`): `/gl-skin` now plays a looping animation clip
  (the bar bends out and back) instead of a static pose. vmesh format v6 adds an
  animation section: per joint, three baked keyframe tracks (translation /
  rotation / scale) with `LINEAR`/`STEP` interpolation. `gltf.zig` parses
  `animations[0]` and bakes one track per joint per channel (an animated channel's
  keys, else a single keyframe of the joint's bind-TRS); `Quat.slerp` does
  shortest-arc rotation interpolation. `GlSkin` samples the tracks at
  `t = elapsed mod duration` each frame → per-joint local TRS → bone palette →
  the existing `set_bones`. A skinned mesh with no clip falls back to the static
  bind pose. **No `command.zig` / shader / wire change** — same `set_bones`
  palette, only its per-frame source differs; all shader + wire goldens stay
  byte-identical (vmesh bumped 5→6, regenerated by the build). Verified animating
  on WebGPU and WebGL2 (headless Chrome, two timed screenshots show different bend
  angles). Deferred: multiple clips / switching, play-pause-speed, CUBICSPLINE.

- **`verve.gl` — skeletal skinning, slice 1 (static pose)** (`src/core/gl/vmesh.zig`
  v5, `command.zig`, `gltf.zig`, `fixture.zig`, `tools/gen_skin_glb.zig`,
  `gl_asset_gen.zig`, `build.zig`, `src/bridge/verve.js`,
  `src/client/islands/GlSkin.zig`, `src/app/{islands,routes,components}.zig`): GPU
  vertex skinning on both backends. Skinned meshes use a stride-56 vertex (the
  stride-48 PBR layout + `joints u8x4` + `weights u8x4`); vmesh format v5 adds a
  `skinned` flag + a skeleton section (`parent`, `inverse_bind`, `bind_local` per
  joint). `gltf.zig` parses `skins[0]` + `JOINTS_0`/`WEIGHTS_0`. A new
  `variant_skinned = 1<<7` PBR shader variant (GLSL + WGSL) blends a 64-entry bone
  palette per vertex (`skin = Σ wᵢ·bones[jointᵢ]`); the palette is uploaded once
  per frame via the new `set_bones` command (wire tag 21) — a separate uniform
  binding (GLSL `u_bones[64]`; WGSL `@group(0) @binding(1)`) so it isn't replicated
  across WebGPU per-draw dynamic-offset slots. A dedicated `GlSkin` chunk + `/gl-skin`
  route render a procedural 3-joint rigged bar deformed by a fixed bent pose,
  verified rendering BENT on WebGPU and WebGL2 (headless Chrome). Non-skinned meshes
  + all existing shader/wire goldens are byte-unchanged (vmesh bumped 4→5,
  regenerated by the build). Keyframe animation is the next slice.
...
- **`verve.gl` — compressed (source-format) textures** (`src/core/gl/vmesh.zig` v4,
  `gltf.zig`, `png.zig`, `tools/gl_asset_gen.zig`, `build.zig`,
  `src/bridge/verve-worker.js`, `verve.js`, `src/client/islands/GlScene.zig`,
  `GlDemo.zig`): vmesh material textures over 64×64 now ship as compressed sibling
  files (`<stem>.tex{N}.<ext>`, PNG) decoded to RGBA off the main thread in the asset
  worker (`createImageBitmap`), instead of baking raw RGBA into the `.vmesh`. A
  per-texture `format` tag keeps small textures raw in-blob (back-compat). The chunk
  streams external textures via `gl_load` and uploads them on the next frame;
  worker-down falls back to main-thread `createImageBitmap`. `png.zig`'s encoder now
  emits real zlib DEFLATE (std `flate`) rather than stored blocks. Net on the demo:
  `demo.vmesh` 265 KB → 2.8 KB + a 10.8 KB `demo.tex0.png` (the 256² base, ~24×
  smaller than raw). No GPU-compressed formats (zero-dep); no shader/wire-golden
  change (vmesh format version bumped 3→4, regenerated by the build).

### Changed

- **`verve.gl` — detailed demo base texture** (`src/core/gl/fixture.zig`): the demo
  cube's base-color map (`pbrCubeGlb` → `demo.vmesh`) is a 256×256 deterministic
  procedural texture (was 8×8 — checker × gradient × per-8×8-block noise × grid
  lines), giving the asset pipeline a real-sized, PNG-compressible texture that the
  compressed-textures pipeline ships externally + decodes off-main.
...
[Unreleased]: https://github.com/sirhco/verve/compare/v0.5.12...HEAD
