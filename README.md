...
- **Geometry compression (`.vmesh` v16)** — index buffers ship compressed with a pure-Zig delta+zigzag+varint codec (**~50% smaller** index sections), decoded losslessly host-side before upload so both backends are unchanged. Lossless → a native round-trip golden is the gate, no GPU verification needed.
