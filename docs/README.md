...
24. [3D engine](24-gl.md) — `verve.gl`: Zig-side scene graph + binary command stream, dumb WebGL2 interpreter in the bridge (zero-copy over linear memory, golden-frozen wire contract); asset pipeline (.glb → packed .vmesh v3 with BVH + name table at build time, pure-Zig PNG decode, hostile-input hardened); unlit + PBR/IBL shader variants; orbit camera, BVH ray-picking, `ctx.glScene` declarative builder + `GlScene` island chunk (poster swap, context-restore Registry replay, GPU lifecycle); P5 verve.anim fusion — `glTarget`/`glTargetRange` tween builders, frozen target-id encoding, setter trampoline, scroll-scrubbed turntable demo; demos at `/gl` and `/gl-scene`
25. [AI tools](25-ai.md) — `verve.ai`: comptime, default-deny tool allowlist over `app.Actions`, JSON Schema generation, `Risk` tiers (safe/mutating/dangerous) with a policy gate + human-confirmation round-trip for dangerous calls, an audited dispatch pipeline shared by HTTP actions and desktop IPC routes, a bounded tool-use agent loop against pluggable providers (live Anthropic Messages API, a scripted `MockProvider` for tests, the Claude Code CLI on desktop), and the `/ai-chat` demo
