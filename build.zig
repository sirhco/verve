...
pub fn build(b: *std.Build) void {
    ...
    for (island_names) |name| {
        ...
        // Per-chunk wasm stack. The linker reserves the bottom of imported
    ...
    // Double-sided demo GLB: an upright MASK quad (doubleSided:true) above a
    // floor plane. Exercises variant_double_sided + cull-off/two-pass paths.
    const gen_double_glb_mod = b.createModule(.{
        .root_source_file = b.path("tools/gen_double_glb.zig"),
        .target = host_target,
        .optimize = optimize,
        .imports = &.{
            .{ .name = "verve_gl", .module = host_gl_mod },
    ...
    const gen_double_glb_exe = b.addExecutable(.{
        .name = "verve-gen-double-glb",
        .root_module = gen_double_glb_mod,
    });
    const gen_double_glb_run = b.addRunArtifact(gen_double_glb_exe);
    const double_glb_path = gen_double_glb_run.addOutputFileArg("double.glb");

    const gl_asset_gen_double_run = b.addRunArtifact(gl_asset_gen_exe);
    gl_asset_gen_double_run.addFileArg(double_glb_path);
    const double_dir = gl_asset_gen_double_run.addOutputDirectoryArg("double");
    gl_asset_gen_double_run.addArg("double");

    // 7×7 cube-grid demo GLB: 49 unit cubes for frustum-cull stress testing.
    const gen_cubegrid_glb_mod = b.createModule(.{
        .root_source_file = b.path("tools/gen_cubegrid_glb.zig"),
        .target = host_target,
        .optimize = optimize,
        .imports = &.{
            .{ .name = "verve_gl", .module = host_gl_mod },
    ...
    const gen_cubegrid_glb_exe = b.addExecutable(.{
        .name = "verve-gen-cubegrid-glb",
        .root_module = gen_cubegrid_glb_mod,
    });
    const gen_cubegrid_glb_run = b.addRunArtifact(gen_cubegrid_glb_exe);
    const cubegrid_glb_path = gen_cubegrid_glb_run.addOutputFileArg("cubegrid.glb");

    const gl_asset_gen_cubegrid_run = b.addRunArtifact(gl_asset_gen_exe);
    gl_asset_gen_cubegrid_run.addFileArg(cubegrid_glb_path);
    const cubegrid_dir = gl_asset_gen_cubegrid_run.addOutputDirectoryArg("cubegrid");
    gl_asset_gen_cubegrid_run.addArg("cubegrid");

    // GPU-instancing demo fixture: gen_cubefield_glb → cubefield.glb → gl_asset_gen → cubefield.vmesh.
    const gen_cubefield_glb_mod = b.createModule(.{
        .root_source_file = b.path("tools/gen_cubefield_glb.zig"),
        .target = host_target,
        .optimize = optimize,
        .imports = &.{
            .{ .name = "verve_gl", .module = host_gl_mod },
    ...
    const gen_cubefield_glb_exe = b.addExecutable(.{
        .name = "verve-gen-cubefield-glb",
        .root_module = gen_cubefield_glb_mod,
    });
    const gen_cubefield_glb_run = b.addRunArtifact(gen_cubefield_glb_exe);
    const cubefield_glb_path = gen_cubefield_glb_run.addOutputFileArg("cubefield.glb");

    const gl_asset_gen_cubefield_run = b.addRunArtifact(gl_asset_gen_exe);
    gl_asset_gen_cubefield_run.addFileArg(cubefield_glb_path);
    ...
    // Instanced-shadow demo fixture: gen_cubeshadow_glb → cubeshadow.glb → gl_asset_gen → cubeshadow.vmesh.
    const gen_cubeshadow_glb_mod = b.createModule(.{
        .root_source_file = b.path("tools/gen_cubeshadow_glb.zig"),
        .target = host_target,
        .optimize = optimize,
        .imports = &.{
            .{ .name = "verve_gl", .module = host_gl_mod },
    ...
    const gen_cubeshadow_glb_exe = b.addExecutable(.{
        .name = "verve-gen-cubeshadow-glb",
        .root_module = gen_cubeshadow_glb_mod,
    });
    const gen_cubeshadow_glb_run = b.addRunArtifact(gen_cubeshadow_glb_exe);
    const cubeshadow_glb_path = gen_cubeshadow_glb_run.addOutputFileArg("cubeshadow.glb");

    const gl_asset_gen_cubeshadow_run = b.addRunArtifact(gl_asset_gen_exe);
    gl_asset_gen_cubeshadow_run.addFileArg(cubeshadow_glb_path);
    const cubeshadow_dir = gl_asset_gen_cubeshadow_run.addOutputDirectoryArg("cubeshadow");
    gl_asset_gen_cubeshadow_run.addArg("cubeshadow");
    ...
    const gen_morph_glb_exe = b.addExecutable(.{
        .name = "verve-gen-morph-glb",
        .root_module = gen_morph_glb_mod,
    });
    const gen_morph_glb_run = b.addRunArtifact(gen_morph_glb_exe);
    const morph_glb_path = gen_morph_glb_run.addOutputFileArg("morph.glb");

    const gl_asset_gen_morph_run = b.addRunArtifact(gl_asset_gen_exe);
    gl_asset_gen_morph_run.addFileArg(morph_glb_path);
    const morph_dir = gl_asset_gen_morph_run.addOutputDirectoryArg("morph");
    gl_asset_gen_morph_run.addArg("morph");

    // Morph16-demo asset: gen_morph16_glb → morph16.glb → gl_asset_gen → morph16.vmesh.
    // 16 targets, each deforming a distinct vertex; cap-32 active set lets all 16 fire.
    const gen_morph16_glb_mod = b.createModule(.{
        .root_source_file = b.path("tools/gen_morph16_glb.zig"),
        .target = host_target,
        .optimize = optimize,
        .imports = &.{
            .{ .name = "verve_gl", .module = host_gl_mod },
    ...
    const gen_morph16_glb_exe = b.addExecutable(.{
        .name = "verve-gen-morph16-glb",
        .root_module = gen_morph16_glb_mod,
    });
    const gen_morph16_glb_run = b.addRunArtifact(gen_morph16_glb_exe);
    const morph16_glb_path = gen_morph16_glb_run.addOutputFileArg("morph16.glb");

    const gl_asset_gen_morph16_run = b.addRunArtifact(gl_asset_gen_exe);
    gl_asset_gen_morph16_run.addFileArg(morph16_glb_path);
    ...
    // LTC LUT asset: a tiny native tool packs the two rect-area-light lookup
    // tables (src/core/gl/ltc_data.zig) into a 65536-byte ltc.bin (ltc_mat ++
    // ltc_mag, both rgba16f). Served at /gl/ltc.bin and fetched by the bridge —
    // kept out of the gl island chunk wasm (chunk-data-window gotcha).
    const gl_ltc_gen_mod = b.createModule(.{
        .root_source_file = b.path("tools/gl_ltc_gen.zig"),
        .target = host_target,
        .optimize = optimize,
        .imports = &.{
            .{ .name = "verve_gl", .module = host_gl_mod },
    ...
    const gl_ltc_gen_exe = b.addExecutable(.{
        .name = "verve-gl-ltc-gen",
        .root_module = gl_ltc_gen_mod,
    });
    // argv shape: <out.bin>. The tool writes the 65536-byte ltc.bin.
    ...
    _ = wf_gl.addCopyFile(cubeshadow_dir.path(b, "cubeshadow.vmesh"), "cubeshadow.vmesh");
    ...
    // The `dracotest.vmesh` embed entry is conditional — it must appear only when
    // the guarded copy above added the file (else `@embedFile` fails on a stripped
    // `tests/`). Injected between the head/tail via `b.fmt` (the literals are %s
    // args, so their `{ }` need no escaping).
    const dracotest_entry: []const u8 = if (dracotest_vmesh != null)
        "\n    .{ .name = \"dracotest.vmesh\", .bytes = @embedFile(\"dracotest.vmesh\") },"
    else
        "";
    const gl_assets_head =
        ...
        \\    .{ .name = "cubeshadow.vmesh", .bytes = @embedFile("cubeshadow.vmesh") },
    ...
    const gl_assets_tail =
        ...
        \\
    ;
    const gl_assets_src = b.fmt("{s},{s}{s}", .{ gl_assets_head, dracotest_entry, gl_assets_tail });
