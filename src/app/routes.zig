...
pub const routes: []const verve.Route = &.{
    ...
    verve.Route.init("/viz-canvas-multi", renderVizCanvasMulti),
    verve.Route.init("/viz-multi", renderVizMulti),
    ...
    verve.Route.init("/gl-draco", renderGlDraco),
    ...
    verve.Route.init("/gl-probe", renderGlProbe),
    ...
    verve.Route.init("/gl-points", renderGlPoints),
    verve.Route.init("/gl-lines", renderGlLines),
    verve.Route.init("/gl-decals", renderGlDecals),
...
fn renderVizCanvasMulti(ctx: *verve.Context) !*verve.Node {
    const body = try components.vizCanvasMulti(ctx);
    return components.page(ctx, body);
}

fn renderVizMulti(ctx: *verve.Context) !*verve.Node {
...
fn renderGlDraco(ctx: *verve.Context) !*verve.Node {
    const body = try components.glSceneDraco(ctx);
    return components.page(ctx, body);
}
...
fn renderGlInstancedMulti(ctx: *verve.Context) !*verve.Node {
    const body = try components.glSceneInstancedMulti(ctx);
    return components.page(ctx, body);
}
...
fn renderGlMorph16(ctx: *verve.Context) !*verve.Node {
    const body = try components.glSceneMorph16(ctx);
    return components.page(ctx, body);
}
...
fn renderGlProbe(ctx: *verve.Context) !*verve.Node {
    const body = try components.glSceneProbe(ctx);
    return components.page(ctx, body);
}
...
fn renderGlPoints(ctx: *verve.Context) !*verve.Node {
    const body = try components.glPoints(ctx);
    return components.page(ctx, body);
}

fn renderGlLines(ctx: *verve.Context) !*verve.Node {
    const body = try components.glLines(ctx);
    return components.page(ctx, body);
}

fn renderGlDecals(ctx: *verve.Context) !*verve.Node {
    const body = try components.glDecals(ctx);
    return components.page(ctx, body);
}

fn renderGlLod(ctx: *verve.Context) !*verve.Node {
    const body = try components.glSceneLod(ctx);
    return components.page(ctx, body);
}

fn renderGlOrtho(ctx: *verve.Context) !*verve.Node {
    const body = try components.glSceneOrtho(ctx);
    return components.page(ctx, body);
}
...
fn renderGlMaterial(ctx: *verve.Context) !*verve.Node {
    const body = try components.glSceneMaterial(ctx);
    return components.page(ctx, body);
}
