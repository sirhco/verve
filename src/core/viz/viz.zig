...
pub const Candle = chart_mod.Candle;
pub const CandleOpts = chart_mod.CandleOpts;
pub const candlestickChart = chart_mod.candlestick;
pub const BoxStats = chart_mod.BoxStats;
pub const boxStats = chart_mod.boxStats;
pub const boxPlotChart = chart_mod.boxPlot;
...
pub const ViolinSeries = chart_mod.ViolinSeries;
pub const violinChart = chart_mod.violin;
...
/// Fixed-buffer `d`-builder (no Allocating writer, wasm function-table-safe).
/// This flat alias is the server-side handle; island chunks reach the same fn
/// via `verve.viz_core.edge_path.pathDBuf` (client_core re-exports the module).
