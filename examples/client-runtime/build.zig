//! Build wiring for the `client-runtime` example. Mirrors the framework's
//! own `build.zig` island pipeline — typed server-fn stubs
//! (`app_client.zig`), the island manifest, and one real per-island WASM
//! chunk per `src/app/islands.zig` decl — so the `JsonProbe` island
//! (which exercises every Phase 17-22 client-runtime primitive) compiles
//! against the shared `verve_island` runtime façade with shared memory +
...
pub fn build(b: *std.Build) void {
    ...
    // Per-island chunk façade — what `@import("verve")` resolves to inside
    // an island source (the thin `verve_runtime` externs, not the full
    // server-side surface).
    ...
    const run_step = b.step("run", "Run the client-runtime server");
    run_step.dependOn(&run_cmd.step);
}

/// Parse `src/app/islands.zig` at configure time to harvest every
/// top-level `pub const <Name> = struct` decl.
